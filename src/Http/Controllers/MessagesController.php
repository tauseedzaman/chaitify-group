...
class MessagesController extends Controller
{
     ...
     * @return \Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View
     */
    public function group_index( $uuid = null)
    {
        $messenger_color = Auth::user()->messenger_color;
        return view('Chatify::pages.app', [
            'id' => $uuid ?? 0,
    ...
    public function send(Request $request)
    {
        ...
        if (!$error->status) {
            $message = Chatify::newMessage([
                ...
                'to_id' => $request['id'],
                'group_id' => Str::isUuid($request['id']) ? $request['id'] : null,
    ...
    public function getContacts(Request $request)
    {
        ...
        // ->groupBy('users.id')
        $users = Message::leftJoin('users', function ($join) {
            $join->on('ch_messages.from_id', '=', 'users.id')
                 ->orOn('ch_messages.to_id', '=', 'users.id');
        })
        ->leftJoin('group_users', function ($join) {
            $join->on('ch_messages.from_id', '=', 'group_users.user_id')
                 ->orOn('ch_messages.to_id', '=', 'group_users.user_id');
        })
        ...
        ->where('users.id', '!=', Auth::user()->id)
        ->orWhere('group_users.user_id', Auth::user()->id)
    ...
    public function createGroup(Request $request)
    {
        ...
        $error = $success = 0;
            ...
            if ($request->file("logo") && $request->name && $request->description) {
            // check file size
                if (in_array(strtolower($file->extension()), $allowed_images)) {
                    // upload
                    $group_name = Str::uuid() . "." . $file->extension();
                    try {
                    ...
                    $message->to_id = $group->id;
                    $message->seen = 1;
                    $message->group_id = $group->id;
                    $message->body = "Welcome your group $request->name";
                    $message->attachment = null;
                    $message->save();
                    DB::commit();


                    $file->storeAs(config('chatify.user_avatar.folder'), $group_name, config('chatify.storage_disk_name'));
                    $success = $group ? 1 : 0;
            ...
            $error=1;
            $msg="please fill the given fields";
        }
