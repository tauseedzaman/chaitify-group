...
class ChatifyMessenger
{
    ...
    public function fetchGroupMessagesQuery($group_uuid)
    {
            return Message::join('users', 'ch_messages.from_id', '=', 'users.id')
            ->where("ch_messages.group_uuid",$group_uuid)
            ->whereNotNull('ch_messages.from_id')
            ->select('ch_messages.*', 'users.name');
            // ->get();

        // return Message::where("group_uuid",$group_uuid)
        // ->whereNotNull('from_id')
        // ->with('sender');
    }
        // return Message::where("group_id",$group_id)->where('from_id', Auth::user()->id)->where('to_id', $group_id)
        // ->orWhere('from_id', $group_id);
    ...
    public function newMessage($data)
    {
        ...
        $message->group_uuid = $data['group_uuid'] ?? null;
    ...
    public function getLastMessageQuery($user_id)
    {
        if(Str::isUuid($user_id)){
            return $this->fetchGroupMessagesQuery($user_id)->latest()->first();
        }else{
    ...
    public function getContactItem($user)
    {
        try {
            ...
            return view('Chatify::layouts.listItem', [
                'get' => 'users',
                'is_group' => Str::isUuid($user->id),
                // 'user' => $user->group_id ? $this->getGroupWithAvatar($user) : $this->getUserWithAvatar($user),
    ...
    public function getGroupWithAvatar($group)
    {
        if ($group->logo== 'avatar.png' && config('chatify.gravatar.enabled')) {
            $imageSize = config('chatify.gravatar.image_size');
            $imageset = config('chatify.gravatar.imageset');
            $group->logo= 'https://www.gravatar.com/avatar/' . md5(strtolower(trim($group->email))) . '?s=' . $imageSize . '&d=' . $imageset;
        } else {
            $group->logo= self::getUserAvatarUrl($group->imagelogo);
        }
        return $group;
    }
    ...
    public function deleteConversation($user_id)
    {
        try {
            ...
            if(Str::isUuid($user_id)){
                $group=Group::where("uuid",$user_id)->first();
    ...
    public function leaveConversation($user_id)
    {
        try {
            foreach ($this->fetchMessagesQuery($user_id)->get() as $msg) {
                // delete file attached if exist
